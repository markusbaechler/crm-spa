...
(() => {
  ...
  const views = {
    ...
    renderBatchEventForm(payload = {}) {
      ...
      const modeLabel = isBestaetigen ? "Teilnahme bestätigen" : "Sammel-Anmeldung";
      const modeDesc = isBestaetigen
        ? `Ausgewählte Kontakte werden von <strong>Event</strong> nach <strong>Eventhistory</strong> verschoben — Teilnahme gilt als bestätigt.`
        : `Ausgewählte Kontakte werden für <strong>${helpers.escapeHtml(eventName)}</strong> angemeldet (Event-Feld gesetzt).`;
            ...
            <form data-modal-form="batch-event" data-event-name="${helpers.escapeHtml(eventName)}" data-mode="${mode}">
              ...
              <div class="bbz-modal-footer">
                ...
                <button type="submit" class="bbz-button bbz-button-primary" ${state.meta.loading || validSelected.length === 0 ? "disabled" : ""}>
                  ${isBestaetigen ? `✓ ${validSelected.length} Teilnahmen bestätigen` : `+ ${validSelected.length} Kontakte anmelden`}
                </button>
              </div>
            </form>
          </div>
        ...
        <div>
          ...
          <div class="bbz-kpis">
            ...
            <div class="bbz-kpi">
              ...
              <div style="margin-top:6px;font-size:12px;color:var(--muted);">A/B-Kunden mit Handlungsbedarf</div>
            </div>
            <div class="bbz-kpi">
            ...
            <div class="bbz-kpi">
              ...
              <div class="bbz-kpi-value" style="color:var(--amber);">${radarColdCount}</div>
              <div style="margin-top:6px;font-size:12px;color:var(--muted);">>360 Tage kein Kontakt</div>
            </div>
            <div class="bbz-kpi">
              <div class="bbz-kpi-label">Überfällige Tasks</div>
              <div class="bbz-kpi-value" style="color:var(--muted);">${radarOverdueCount}</div>
              <div style="margin-top:6px;font-size:12px;color:var(--muted);">A/B-Kunden betroffen</div>
            </div>
            ` : `
    ...
    async handleBatchEventSubmit(form) {
      ...
      try {
        ...
        const results = await Promise.allSettled(selectedIds.map(async cid => {
          ...
          if (isBestaetigen) {
            // Event aus Event-Feld entfernen, in Eventhistory eintragen
            newEvent     = newEvent.filter(e => String(e).trim() !== eventName);
            if (!newEventHist.includes(eventName)) newEventHist = [...newEventHist, eventName];
          } else {
            // Event anmelden: Event-Feld ergänzen
        ...
        const msg = isBestaetigen
          ? `✓ ${ok} Teilnahme${ok !== 1 ? "n" : ""} bestätigt${fail > 0 ? ` — ${fail} Fehler (Konsole prüfen)` : ""}.`
          : `✓ ${ok} Kontakt${ok !== 1 ? "e" : ""} angemeldet${fail > 0 ? ` — ${fail} Fehler (Konsole prüfen)` : ""}.`;
