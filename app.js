...
// State für Daten UND Metadaten (Dropdowns)
...
async function loadData() {
    ...
    content.innerHTML = `<div class="p-20 text-center text-slate-400 font-bold uppercase tracking-widest animate-pulse">Synchronisiere Metadaten...</div>`;
    try {
        ...
        // 2. Metadaten (Dropdown-Optionen) direkt aus den SharePoint Spalten laden
        ...
        <div class="max-w-[1600px] mx-auto animate-in fade-in">
            <div class="flex justify-between items-center mb-6 border-b pb-4">
                ...
                <div class="flex gap-3">
                    <input type="text" onkeyup="filterFirms(this.value)" placeholder="Firma suchen..." class="px-4 py-2 border rounded-lg text-sm w-64 outline-none focus:border-blue-500 shadow-sm">
            ...
            <div id="addForm" class="hidden mb-8 p-6 bg-slate-50 border rounded-2xl shadow-inner animate-in slide-in-from-top">
                ...
                <button onclick="saveNewFirm()" class="bg-green-600 text-white font-bold text-xs px-6 py-2 rounded uppercase tracking-widest shadow-md">Firma Speichern</button>
            </div>
            ...
            <div class="bg-white border rounded-xl overflow-hidden shadow-sm">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody id="firmTableBody" class="divide-y divide-slate-100">
                        ${allFirms.map(f => `
                            <tr onclick="renderDetail('${f.id}')" class="hover:bg-blue-50 cursor-pointer transition-colors group">
                                ...
                                <td class="p-4 text-center font-bold text-[10px] italic">${f.fields.Klassifizierung || '-'}</td>
        ...
        <div class="max-w-[1600px] mx-auto animate-in slide-in-from-right duration-300">
            ...
            <div class="bg-white border rounded-xl shadow-sm overflow-hidden">
                <div class="p-4 bg-slate-50 border-b flex justify-between items-center">
                    ...
                    <button onclick="toggleContactForm()" class="bg-blue-600 text-white px-3 py-1.5 rounded text-[9px] font-bold uppercase shadow-sm tracking-widest">+ Neuer Kontakt</button>
                </div>
                ...
                <div id="addContactForm" class="hidden p-8 bg-blue-50/50 border-b animate-in fade-in">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                        ...
                        <div><label class="text-[9px] font-bold uppercase text-slate-400">E-Mail geschäftlich</label><input type="text" id="c_email1" class="w-full p-2 border rounded text-sm"></div>
                        <div><label class="text-[9px] font-bold uppercase text-slate-400">Direktwahl</label><input type="text" id="c_dw" class="w-full p-2 border rounded text-sm"></div>
                        <div><label class="text-[9px] font-bold uppercase text-slate-400">Mobile</label><input type="text" id="c_mo" class="w-full p-2 border rounded text-sm"></div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div><label class="text-[9px] font-bold uppercase text-slate-400">SGF</label>
                            <select id="c_sgf" class="w-full p-2 border rounded bg-white text-sm uppercase">
                                <option value="">-</option>${meta.sgf.map(o => `<option value="${o}">${o}</option>`).join('')}
                            </select>
                        </div>
                        <div><label class="text-[9px] font-bold uppercase text-slate-400">Lead bbz</label>
                            <select id="c_lead" class="w-full p-2 border rounded bg-white text-sm uppercase">
                                <option value="">-</option>${meta.leads.map(o => `<option value="${o}">${o}</option>`).join('')}
                            </select>
                        </div>
                        <div class="md:col-span-2 flex items-end gap-3">
                            <button onclick="saveContact('${id}')" class="flex-1 bg-blue-600 text-white font-bold text-xs py-3 rounded uppercase tracking-widest shadow-md">Kontakt in SharePoint Speichern</button>
                            <button onclick="toggleContactForm()" class="px-6 py-3 text-slate-400 font-bold text-xs uppercase">Abbrechen</button>
                        </div>
                    </div>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ${contacts.map(c => `
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="p-4"><span class="px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded border border-emerald-100 font-black uppercase text-[9px]">${c.fields.SGF || '-'}</span></td>
            ...
            <div id="editSide" class="fixed inset-y-0 right-0 w-[400px] bg-white shadow-2xl z-50 transform translate-x-full transition-transform p-8 border-l italic">
                 <div class="flex justify-between items-center mb-8 border-b pb-4"><h3 class="font-black uppercase text-sm">Unternehmensprofil</h3><button onclick="document.getElementById('editSide').classList.toggle('translate-x-full')">✕</button></div>
                 <div class="space-y-4">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase">Firma</label><input type="text" id="e_t" value="${f.Title}" class="w-full p-2 border rounded-lg text-sm font-bold italic">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase">Klassifizierung</label><select id="e_k" class="w-full p-2 border rounded-lg text-sm">${meta.klassen.map(o => `<option value="${o}" ${f.Klassifizierung===o?'selected':''}>${o}</option>`).join('')}</select>
                    ...
                    <label class="block text-[10px] font-bold text-slate-400 uppercase">Adresse</label><input type="text" id="e_s" value="${f.Adresse||''}" class="w-full p-2 border rounded-lg text-sm mb-2 italic">
                    <div class="flex gap-2"><input type="text" id="e_c" value="${f.Ort}" class="flex-1 p-2 border rounded-lg text-sm font-bold italic"><input type="text" id="e_l" value="${f.Land||'CH'}" class="w-16 p-2 bg-slate-100 border rounded-lg text-xs text-center font-bold"></div>
                    <button onclick="updateFirmData('${id}')" class="w-full bg-slate-900 text-white py-3 rounded-lg font-bold text-xs uppercase tracking-widest mt-8 shadow-lg">Speichern</button>
                 </div>
            </div>
        </div>`;
}
...
async function saveContact(firmId) {
    ...
    const fields = { 
        Title: nn, Vorname: document.getElementById('c_vn').value, Anrede: document.getElementById('c_anrede').value,
        Email1: document.getElementById('c_email1').value, Mobile: document.getElementById('c_mo').value,
        Direktwahl: document.getElementById('c_dw').value, Funktion: document.getElementById('c_fun').value,
        Rolle: document.getElementById('c_rolle').value, SGF: document.getElementById('c_sgf').value,
        Leadbbz0: document.getElementById('c_lead').value, FirmaLookupId: firmId 
    };
    const token = (await msalInstance.acquireTokenSilent({ ...loginRequest, account: msalInstance.getAllAccounts()[0] })).accessToken;
...
function filterFirms(q) { const ql = q.toLowerCase(); const f = allFirms.filter(x => x.fields.Title?.toLowerCase().includes(ql) || x.fields.Ort?.toLowerCase().includes(ql)); document.getElementById('firmTableBody').innerHTML = f.map(item => ` <tr onclick="renderDetail('${item.id}')" class="hover:bg-blue-50 cursor-pointer transition-colors"><td class="p-4 font-bold text-slate-700">${item.fields.Title} ${item.fields.VIP ? '⭐' : ''}</td><td class="p-4 text-slate-500 font-medium">${item.fields.Ort || '-'}</td><td class="p-4 text-center font-bold text-[10px] italic">${item.fields.Klassifizierung || '-'}</td><td class="p-4 text-center text-blue-600 font-bold">${allContacts.filter(c => String(c.fields.FirmaLookupId) === String(item.id)).length}</td></tr>`).join(''); }
